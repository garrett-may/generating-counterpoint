...
# Populates note frequencies for a song
def populate_notes_per_notes(song):
    # Initial information
    note_types = util.note_names #[note_name + str(octave) for note_name in util.note_names for octave in range(0, 8)]
    chord_types = util.chord_names
...
def read_notes_corpus(corp, debug=False):
    ...
    # All
    (unigrams, bigrams, trigrams, tetragrams, given) = corpus.read_corpus(corp, populate_notes_per_notes, filt=None, debug=debug)
   
    transform.export_JSON('json/notes_2_given.json', given)
        
    # Major
    (unigrams, bigrams, trigrams, tetragrams, given) = corpus.read_corpus(corp, populate_notes_per_notes, filt=lambda song: is_major(song), debug=debug)
   
    transform.export_JSON('json/notes_2_major_given.json', given)

    # Minor
    (unigrams, bigrams, trigrams, tetragrams, given) = corpus.read_corpus(corp, populate_notes_per_notes, filt=lambda song: not is_major(song), debug=debug)
   
    transform.export_JSON('json/notes_2_minor_given.json', given)
   
def algorithm(melody, chords, algorithm):
    is_major = True
    ...
    (with_octaves_unigrams, given_2) = (notes_with_octaves_major_unigrams, notes_2_major_given) if is_major else \
                                        (notes_with_octaves_minor_unigrams, notes_2_minor_given)
    ...
    for note_1 in note_types:
        V[0][note_1] = {'prob': unigrams[note_1] * given[note_1][chords[0]] * given_2[note_1][melody[0].name], 'prev': None}
    ...
    for note_2 in note_types:
        ...
        V[1][note_2] = {'prob': max_tr_prob * given[note_2][chords[1]] * given_2[note_1][melody[1].name], 'prev': n_1}
    ...
    for note_3 in note_types:
        ...
        V[2][note_3] = {'prob': max_tr_prob * given[note_3][chords[2]] * given_2[note_1][melody[2].name], 'prev': n_2}    
    ...
    for t in range(3, len(chords)):
        ...
        for note_4 in note_types:
            ...
            (n_3, max_tr_prob) = viterbi.rand_probability(tr_probs)
            V[t][note_4] = {'prob': max_tr_prob * given[note_4][chords[t]] * given_2[note_1][melody[t].name], 'prev': n_3}
          
    return [viterbi.rand_probability([(note_2, with_octaves_unigrams[note_2]) for note_2 in [note_1 + str(octave) for octave in range(0, 8)]])[0] for note_1 in viterbi.max_backtrace(V, debug=True)]                                           
