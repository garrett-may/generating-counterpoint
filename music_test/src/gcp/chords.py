...
def read_chords_corpus(composer='bach', debug=False):
    ...
    # All
    (unigrams, bigrams, trigrams, given) = corpus.read_corpus(composer, corpus.populate_chords, filt=None, debug=debug)
    ...
    # Major
    (unigrams, bigrams, trigrams, given) = corpus.read_corpus(composer, corpus.populate_chords, filt=lambda song: is_major(song), debug=debug)
    ...
    # Minor
    (unigrams, bigrams, trigrams, given) = corpus.read_corpus(composer, corpus.populate_chords, filt=lambda song: not is_major(song), debug=debug)
